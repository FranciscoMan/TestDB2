...
-- Description: Get values of stack light units
-- =============================================
-- 03/03/2020 : Minor changes in flags. by AA
-- =============================================

CREATE PROCEDURE    [PRD].[SPE_GET_STACK_LIGHT_VALUES]
...
-- Si no hay, se rellenan con 0.
  ...
  -- SELECT * FROM @T_PRODUCTION_LINE -- PRUEBAS
	...
	SELECT ID_PRODUCTION_LINE
		...
		, NO_YELLOW_REGISTER
		, NO_GRAY_REGISTER
		, CASE WHEN  FG_LINE_DOWN = 1 
		        THEN CAST(1 AS BIT) ELSE CAST(0 AS BIT) END AS NO_RED_VALUE --RED
		, CASE WHEN FG_LINE_DOWN = 0 AND FG_WORK_ORDERS = 1 AND  FG_RECLAIMED_SHIFT = 1 AND FG_LOST_TIME_ATTENDED = 1 AND FG_LINE_RUNNING = 0
		...
		, CASE WHEN FG_LINE_DOWN = 0  AND  FG_RECLAIMED_SHIFT = 0 AND FG_LINE_RUNNING = 1 -- AND FG_WORK_ORDERS = 1
