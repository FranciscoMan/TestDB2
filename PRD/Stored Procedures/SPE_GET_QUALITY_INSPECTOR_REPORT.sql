...
-- =============================================
CREATE PROCEDURE    [PRD].[SPE_GET_QUALITY_INSPECTOR_REPORT]
   @NO_WORK_ORDER INT
   , @DT_WORK_ORDER DATE
...
--GET FLAG EXISTS REDING
	...
	INSERT INTO @TB_SKIDS_PER_WO (ID_INSPECTION_SKID)
	...
	WHERE @NO_WORK_ORDER IS NULL OR( WO.NO_WORK_ORDER = @NO_WORK_ORDER)
	...
	FROM (
		...
		FROM (
			SELECT KF.ID_K_FORM
				  ,KF.ID_INSPECTION_SKID AS ID_PALLET
				  ,KF.ID_FORM
				  ,KF.KY_PROCESS_TYPE
				  ,KF.KY_STATUS_FORM
			  FROM PRD.K_FORM KF
			  JOIN @TB_SKIDS_PER_WO KP ON KF.ID_INSPECTION_SKID = KP.ID_INSPECTION_SKID
	...
	SELECT CIC.ID_METRICS
		...
		, KF.ID_K_FORM
		, KF.ID_INSPECTION_SKID --, KF.ID_PALLET
	...
	FROM(
		SELECT
		      KWO.ID_WORK_ORDER
			, KWO.NO_WORK_ORDER
		    , KIS.ID_INSPECTION_SKID AS ID_PALLET
			, KIS.DT_CREATION DT_INITIAL_TIME
			, '' KY_SHIFT_TIME 
			, KIS.NO_PALLET
			, KIS.NO_QUANTITY
			, CASE WHEN VCPS.FG_FOR_SAVE = 1 THEN KIS.NO_QUANTITY ELSE 0 END AS NO_ACCEPTED
			, CASE WHEN VCPS.FG_FOR_SAVE = 0 THEN KIS.NO_QUANTITY ELSE 0 END AS NO_REJECTED
		...
		FROM PRD.K_INSPECTION_SKID KIS
			...
			INNER JOIN ADM.C_BRANCH_PLANT BP ON KWO.ID_BRANCH_PLANT = BP.ID_BRANCH_PLANT
			INNER JOIN ADM.VW_C_PALLET_STATUS VCPS
				ON KIS.KY_STATUS = VCPS.KY_PALLET_STATUS
			LEFT JOIN @TB_SKIDS_READ_MANUFACTURE SRM ON KIS.ID_INSPECTION_SKID = SRM.ID_INSPECTION_SKID
			LEFT JOIN ( SELECT * FROM  @TB_K_FORM_IN_SPECS S WHERE S.KY_IN_NOMINAL_RANGE = 'NO') MIS ON KIS.ID_INSPECTION_SKID = MIS.ID_PALLET
			LEFT JOIN (SELECT * FROM @TB_RESULT_FORM_MANUALLY WHERE DO_MANUALLY = 'M') DM ON KIS.ID_INSPECTION_SKID = DM.ID_PALLET
		WHERE (@NO_WORK_ORDER IS NULL OR( KWO.NO_WORK_ORDER = @NO_WORK_ORDER))
			AND (@DT_WORK_ORDER IS NULL OR (@DT_WORK_ORDER = CAST(KIS.DT_CREATION AS DATE)))
		)RES 
		GROUP BY 
			  ID_WORK_ORDER
			, NO_WORK_ORDER
			...
			, ID_BRANCH_PLANT
			, NM_BRANCH_PLANT
	 ORDER BY DT_INITIAL_TIME DESC, NO_WORK_ORDER ASC, NO_PALLET ASC
