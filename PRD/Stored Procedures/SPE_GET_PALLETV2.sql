...
-- CREATE date: 03/17/2020
-- Description: get all pallets on hold or/and back_to_review.
-- =============================================
---- 25/05/2020: Fixed on hold. by AA

CREATE PROCEDURE    [PRD].[SPE_GET_PALLETV2]
...
--******* GET POSITION OF LEADMEN FROM CONFIGURATION ********		
	...
	SELECT KP.ID_PALLET	
		...
		, KP.NO_QUANTITY
		, KP.DT_INITIAL_TIME
		, KIS.DT_UPDATE AS DT_FINAL_TIME
		, KP.KY_STATUS
		, CPS.NM_PALLET_STATUS
		, KIS.NM_QUALITY_INSPECTOR AS HOLD_BY
		, KIS.DS_EXPLANATION_AGREEMENT AS CAUSE
		...
		, PRD.F_GET_USERS_BY_POSITION(KP.ID_FIRST_LEVEL_EMPLOYEE, KP.ID_SECOND_LEVEL_EMPLOYEE_REJECTION,
		                              KP.ID_THIRD_LEVEL_EMPLOYEE_REJECTION) AS XML_USERS
		, WO.ID_BRANCH_PLANT
		, KP.FG_SEND_FORM
	FROM PRD.K_PALLET KP
		...
		INNER JOIN PRD.K_INSPECTION_SKID  KIS 
		    ON KIS.ID_WORK_ORDER = KP.ID_WORK_ORDER AND KP.NO_PALLET = KIS.NO_PALLET
	...
	WHERE 
	     (@PIN_ID_WORK_ORDER IS NULL OR (@PIN_ID_WORK_ORDER IS NOT NULL AND KIS.ID_WORK_ORDER = @PIN_ID_WORK_ORDER))
		AND KP.KY_STATUS IN (SELECT KY_PALLET_STATUS FROM ADM.VW_C_PALLET_STATUS  VWPS WHERE VWPS.KY_TEMP_STATUS = 'H')
