...
-- =============================================
CREATE FUNCTION [PRD].[F_GET_WORK_ORDER_FROM_JDEDWARDS] (@PIN_ID_WORK_ORDER INT) 
RETURNS TABLE 
AS
	RETURN 
	(
		SELECT ID_WORK_ORDER
			, KY_CUSTOMER
			, ISNULL(NM_CUSTOMER, 'PLASKOLITE') AS NM_CUSTOMER
			, NO_WORK_ORDER
			, NO_ORDER
			, NO_SEQ
			, ID_ITEM
		...
		FROM JDEPROD..PRODDTA.F4801 W
			LEFT JOIN JDEPROD..PRODDTA.F0101 CN
				ON W.WAAN8 = CN.ABAN8
		...
		WHERE W.WADOCO = @PIN_ID_WORK_ORDER
	) AS WO
	)
